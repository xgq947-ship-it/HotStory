...
class AutoCrawler(CrawlerProvider):
    ...
    def __init__(self, settings: Settings) -> None:
        self.simple = SimpleHttpCrawler(settings)
...
def create_crawler_provider(settings: Settings | None = None) -> CrawlerProvider:
    settings = settings or get_settings()
    provider = settings.crawler_provider.lower().strip()
    if provider == "auto":
        return AutoCrawler(settings)
    if provider in {"simple", "simple_http"}:
        return SimpleHttpCrawler(settings)
